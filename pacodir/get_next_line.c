...
char* fill_line(char* line, char (*stash)[BUFFER_SIZE], int *index, int pos){
	...
	*index = ft_stash(FIND_INDEX, stash, 0);
	if (pos == -1)
	{
		line = ft_concat(line, *stash + *index, BUFFER_SIZE - *index, LINE_MAX_SIZE);
		if (!line) return (free(line), ft_stash(SET_INDEX, stash, BUFFER_SIZE), NULL);
		ft_stash(SET_INDEX, stash, BUFFER_SIZE);
		*index = 0;
	} else
	{
		substring = ft_substr(*stash, BUFFER_SIZE, *index, pos - *index + 1);
		if (!substring) return (free(line), ft_stash(SET_INDEX, stash, BUFFER_SIZE), NULL);
		line = ft_concat(line, substring, ft_strlen(substring), LINE_MAX_SIZE);
		if (!line) return (ft_stash(SET_INDEX, stash, BUFFER_SIZE), free(substring), NULL);
...
char *get_next_line(int fd)
{
	...
	if (line == NULL) return (ft_stash(SET_INDEX, &stash, BUFFER_SIZE), NULL);
	line[0] = '\0';
	while (!ft_stash(EMPTY, &stash, 0) || ((n_read = read(fd, stash, BUFFER_SIZE))))
	{
		if (n_read < 0) break;
	...
	if (line[0] == '\0' || n_read < 0) return (free(line), NULL);
